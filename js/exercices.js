// Catalogue des exercices de mathématiques — niveau CE2.
...
export const moduleParId = (id) => MODULES.find((m) => m.id === id);

// Fabrique une série d'exercices variés, sans doublon d'énoncé.
export function serie(moduleIds, nb, difficulte) {
  const out = [];
  const vus = new Set();
  let garde = 0;
  while (out.length < nb && garde++ < nb * 40) {
    const id = moduleIds[out.length % moduleIds.length];
    const mod = moduleParId(id);
    const ex = mod.gen(difficulte(id));
    if (vus.has(ex.enonce)) continue;
    vus.add(ex.enonce);
    out.push({ ...ex, moduleId: id });
  }
  return out;
}
